...
class DATA_LOADER():
	def __init__(self, n_questions, seqlen, seperate_char, name='data'):
	...
	def load_data(self, path):
		...
		for lineid, line in enumerate(f_data):
			...
			for k in xrange(n_split):
				...
				if k == n_split - 1:
					endindex = len(answer_list)
				else:
					endindex = (k+1)*self.seq_len
				for i in xrange(k*self.seq_len, end_index):
					...
					print('Question tag : %d, Answer : %d, QA : %d' %(q_tag_list[i], answer_list[i], qa_values))
