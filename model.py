...
class Model():
	...
	def create_model():
		...
		self.memory = DKVMN(self.args.memory_size, self.args.memory_key_state_dim, \
				self.args.memory_value_state_dim, init_memory_key, init_memory_value)
		...
		with tf.variable_scope('Embedding'):
			...
			q_embed_mtx = tf.get_variable('q_embed', [self.args.n_questions, self.args.memory_key_state_dim],\
				initializer=tf.truncated_normal_initializer(stddev=0.1))
			# B
			qa_embed_mtx = tf.get_variable('qa_embed', [2*self.args.n_questions, self.args.memory_value_state_dim], initializer=tf.truncated_normal_initializer(stddev=0.1))		
		...
		slice_q_embed_data = tf.split(q_embed_data, num_or_size_splits=self.args.seq_len, axis=1)
		...
		slice_qa_embed_data = tf.split(qa_embed_data, num_or_size_splits=self.args.seq_len, axis=1)
		...
		for i in xrange(self.args.seq_len):
			...
			q = tf.squeeze(slice_q_embed_data[i])
			...
			qa = tf.squeeze(slice_qa_embed_data[i])
		...
		self.pred = tf.stack(prediction, axis=1)
		# Define loss : standard cross entropy loss
		self.loss = tf.reduced_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.target, logits=self.pred))

		# Optimizer : SGD + MOMENTUM with learning rate decay
		global_step = tf.Variable(0, trainable=False)
		lr = tf.train.exponential_decay(self.args.initial_lr, global_step=global_step, decay_steps=2000, decay_rate=0.667, stair_case=True)
		...
		self.train_op = optimizer.apply_gradients(grad_clip, global_step=global_step)
	...
	def train(self, train_q_data, train_qa_data, valid_q_data, valid_qa_data, label):
		...
		for epoch in xrange(self.train_count, self.args.num_epochs):
			...
			for steps in xrange(training_step):
				...
				epoch_loss += loss
				print('Epoch %d/%d, steps %d/%d, loss : %3.5f' % (epoch+1, self.args.num_epochs, steps+1, training_step, loss))
